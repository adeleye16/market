...
def profile_update(request):
      ...
      if request.method == 'POST':
            ...
            if update.is_valid():
                  user = update.save()
                  new = user.first_name
                  messages.success(request, f'congratulations {{new}}, Your profile updateis successful')
...
def cart(request):
      if request.method == 'POST':
            ...
            if cart:
                  basket = Cart.objects.filter(product=product.id, user__username = request.user.username, paid=False).first()
                  if basket:
...
def deleteitem(request):
      if request.method == 'POST':
            ...
            item_delete = Cart.objects.get(pk=items_id)
            item_delete.delete()
...
def change(request):
      if request.method == 'POST':
            ...
            modify.save()
            messages.success(request, 'Cart modify successfully')
      return redirect('shopcart')
...
def pay(request):
      if request.method == 'POST':
            api_key = 'sk_test_c4102a30afc266ebbc7ee0df269495f6140e1230'
            curl = 'https://api.paystack.co/transaction/initialize'
            cburl = 'http://52.87.159.210/callback/'
...
def history(request):
      ...
      # category = Category.object.all() = query
